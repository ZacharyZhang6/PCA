...
def get_train_mse(R, user_vecs, movie_vecs):
    ...
    for i in range(R.shape[0]):
        for j in range(R.shape[1]):
            if not np.isnan(R[i, j]):
                mse_loss += (user_vecs[i] @ movie_vecs[j] - R[i,j]) ** 2
    return mse_loss
...
def update_user_vecs(user_vecs, movie_vecs, R, user_rated_idxs):
    ...
    for i in range(R.shape[0]):
        index  = user_rated_idxs[i]
        temp_inv = np.matrix(movie_vecs[index]).T.dot(np.matrix(movie_vecs[index])) + np.identity(movie_vecs.shape[1])
        temp = R[i,index] @ movie_vecs[index]
        user_vecs[i] = np.linalg.inv(temp_inv) @ temp
    return user_vecs
...
def update_movie_vecs(user_vecs, movie_vecs, R, movie_rated_idxs):
    ...
    for i in range(R.shape[1]):
        index = movie_rated_idxs[i]
        temp_inv = np.matrix(user_vecs[index]).T.dot(np.matrix(user_vecs[index])) + np.identity(user_vecs.shape[1])
        temp = R[index,i] @ user_vecs[index]
        movie_vecs[i] = np.linalg.inv(temp_inv) @ temp
    return movie_vecs
